...
void displayBestiaires(EspecePokemon bestiaire[])
{
	for (int i=0; i < 150; i++)
	{
		cout << "les information de bestiaire[" << i << "] : " << endl;
		...
		{
			cout << " Evolution MAX" << endl;
		}
		else cout << bestiaire[i].evolvesTo << endl;
		cout << "=========================================" << endl;
	}
}

void triBestiaires(EspecePokemon bestiaire0[])
{
	EspecePokemon bestiaire[150];
	for (int i = 0; i < 150; i++)
	{
		bestiaire[i] = bestiaire0[i];
	}
	EspecePokemon temps;
	for (int i = 1; i <= 150; i++)
		for(int j=1; j<=150-i; j++)
	{
		if (strcmp(bestiaire[j].nom,bestiaire[j-1].nom) <=0)
		{
			temps = bestiaire[j - 1];
			bestiaire[j - 1] = bestiaire[j];
			bestiaire[j] = temps;
		}
		
	}
	displayBestiaires(bestiaire);
}
